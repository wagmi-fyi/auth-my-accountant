import { notFound } from "next/navigation";
import { eq } from "drizzle-orm";
import Image from "next/image";
import { db } from "@/lib/db";
import { bundles } from "@/lib/schema";
import BundleAuthFlow from "./BundleAuthFlow";

interface ConsentData {
  title: string;
  body: string;
  firm_name: string;
}

interface AccountResult {
  provider_account_id: string;
  account_metadata: Record<string, string>;
}

interface SessionEntry {
  session_id: string;
  client_secret: string;
  status: string;
  accounts: AccountResult[];
}

function PageShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full">{children}</div>
      <div className="mt-12 opacity-40">
        <Image src="/icon.svg" alt="" width={24} height={24} />
      </div>
    </div>
  );
}

export default async function BundleAuthPage({
  params,
}: {
  params: Promise<{ token: string }>;
}) {
  const { token } = await params;

  const [bundle] = await db
    .select()
    .from(bundles)
    .where(eq(bundles.token, token));

  if (!bundle) {
    notFound();
  }

  const consent = bundle.consent as ConsentData;
  const sessions = bundle.sessions as SessionEntry[];

  // Expired
  if (new Date(bundle.expiresAt) < new Date()) {
    return (
      <PageShell>
        <div className="text-center">
          <Image
            src="/icon.svg"
            alt=""
            width={48}
            height={48}
            className="mx-auto mb-4 opacity-30"
          />
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">
            Link Expired
          </h1>
          <p className="text-gray-600">
            This link has expired. Please contact your accountant for a new
            link.
          </p>
        </div>
      </PageShell>
    );
  }

  // Completed — show all connected accounts
  if (bundle.status === "completed") {
    const allAccounts = sessions.flatMap((s) =>
      s.status === "completed" ? s.accounts : []
    );

    return (
      <PageShell>
        <div className="text-center">
          <Image
            src="/icon.svg"
            alt=""
            width={48}
            height={48}
            className="mx-auto mb-4"
          />
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">
            Already Connected
          </h1>
          <p className="text-gray-600 mb-6">
            Your accounts have already been connected successfully.
          </p>
          {allAccounts.length > 0 && (
            <div className="bg-gray-50 rounded-lg p-4 text-left">
              <p className="text-sm font-medium text-gray-700 mb-2">
                Connected accounts:
              </p>
              {allAccounts.map((a) => (
                <div
                  key={a.provider_account_id}
                  className="text-sm text-gray-600 py-1 border-b border-gray-100 last:border-0"
                >
                  {a.account_metadata?.institution_name || "Account"}{" "}
                  {a.account_metadata?.last4
                    ? `****${a.account_metadata.last4}`
                    : ""}
                </div>
              ))}
            </div>
          )}
        </div>
      </PageShell>
    );
  }

  // Pending or Active — show consent + auth flow
  const sessionData = sessions.map((s, index) => ({
    index,
    client_secret: s.client_secret,
    status: s.status,
    accounts: s.accounts,
  }));

  return (
    <PageShell>
      <div className="text-center mb-8">
        <Image
          src="/icon.svg"
          alt=""
          width={36}
          height={36}
          className="mx-auto mb-3"
        />
        <p className="text-sm text-gray-500 mb-1">Requested by</p>
        <h2 className="text-lg font-semibold text-gray-900">
          {consent.firm_name}
        </h2>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
        <h1 className="text-xl font-semibold text-gray-900 mb-3">
          {consent.title}
        </h1>
        <p className="text-gray-600 text-sm leading-relaxed whitespace-pre-wrap">
          {consent.body}
        </p>
      </div>

      <BundleAuthFlow
        bundleId={bundle.id}
        bundleToken={bundle.token}
        provider={bundle.provider}
        publishableKey={bundle.providerPublishableKey || ""}
        sessions={sessionData}
      />
    </PageShell>
  );
}
